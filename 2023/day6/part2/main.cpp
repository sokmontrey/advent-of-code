...
int code(string input) {
  ...
  auto time = toLongLong(
      vectorReduce<string, string>(split(split(lines[0], ": ")[1], " "), "",
                                   [](string a, string b) { return a + b; }));
  auto distance = toLongLong(
      vectorReduce<string, string>(split(split(lines[1], ": ")[1], " "), "",
                                   [](string a, string b) { return a + b; }));

  auto root = quadraticRoots<double>(-1, time, -distance);
